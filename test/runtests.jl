...
test_logdensity(x::AbstractVector{T}) where {T} =
    any(x .< 0) ? -T(Inf) : test_logdensity1(x)
...
ForwardDiff.checktag(
    ::Type{ForwardDiff.Tag{TestTag,V}},
    ::Base.Fix1{typeof(logdensity),typeof(TestLogDensity())},
    ::AbstractArray{V},
) where {V} = true
